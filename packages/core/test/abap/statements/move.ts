...
const versions = [
  {abap: "lo_foo = NEW zcl_class( ).", rel: Release.v740sp02},
  {abap: "lo_obj = CAST cl_abap_objectdescr( cl_abap_objectdescr=>describe_by_object_ref( ii_handler ) ).", rel: Release.v740sp02},
  {abap: "DATA(lo_obj) = CAST cl_abap_objectdescr(\n cl_abap_objectdescr=>describe_by_object_ref( ii_handler ) ).", rel: Release.v740sp02},
  {abap: "foo = CORRESPONDING #( get( ) ).", rel: Release.v740sp05},
  {abap: "lv_commit = CONV #( iv_branch ).", rel: Release.v740sp02},
  {abap: "lv_value = REF #( attribute_value ).", rel: Release.v740sp02},
  {abap: "ev_filename = mt_files[ 1 ]-key-obj_name.", rel: Release.v740sp02},
  {abap: "DATA(new_level) = VALUE #( mt_levels[ random_int ] OPTIONAL ).", rel: Release.v740sp08},
  {abap: "land_text = countries[ land1 = lv_foo ]-landx50.", rel: Release.v740sp02},
  {abap: "DATA(lt_tkey) = VALUE cvt_selops( ( option = 'EQ' sign   = 'I' low    = 'sdf' ) ).", rel: Release.v740sp02},
  {abap: "lv_xstr = CAST cl_sxml_string_writer( li_writer )->get_output( ).", rel: Release.v740sp02},
  {abap: "lv_value = COND #( WHEN type_kind = 'r' THEN 's' WHEN type_kind = 'h' THEN 'b' ELSE 'sd' ).", rel: Release.v740sp02},
  {abap: "moo = COND string( WHEN 1 = 2 THEN '12' ELSE '34' ) && 'bar'.", rel: Release.v740sp02},
  {abap: "lv_commit = lo_repo->get_branch( CONV #( iv_branch ) )->get_data( )-sha1.", rel: Release.v740sp02},
  {abap: "foo = gt_det[ <lv_row> ].", rel: Release.v740sp02},
  {abap: "foo = li_foo->create( VALUE #( ) ).", rel: Release.v740sp02},
  {abap: "foo = mi_foo->update( CORRESPONDING #( get( ) EXCEPT field ) ).", rel: Release.v740sp05},
  {abap: "foo = VALUE /bobf/t_frw_name( ).", rel: Release.v740sp02},
  {abap: "foo = switch #( i_popup when abap_true then c_popup when abap_false then c_full ).", rel: Release.v740sp02},
  {abap: "_locale = SWITCH #( i_locale WHEN `` THEN get_locale( ) ELSE i_locale ).", rel: Release.v740sp02},
  {abap: "r_source = VALUE #( BASE r_source ( source_line ) ).", rel: Release.v740sp02},
  {abap: "foo = VALUE #( ( col1 = 13 col2 = 232 col3 = 332 ) ).", rel: Release.v740sp02},
  {abap: "excluded = value #( ( b_01 ) ).", rel: Release.v740sp02},
  {abap: "excluded = value #( ( foo = b_01 ) ( bar = b_02 ) ).", rel: Release.v740sp02},
  {abap: "excluded = value #( ( b_01 ) ( b_02 ) ).", rel: Release.v740sp02},
  {abap: "DATA(lt_tadir) = VALUE tt_tadir( FOR ls_object IN it_objects (\n" +
    "pgmid = ls_object-pgmid\n" +
    "object = ls_object-object\n" +
    "obj_name = ls_object-obj_name ) ).", rel: Release.v740sp05},
  {abap: "foo = VALUE #(\n" +
    ...
    "( col1 = 13 col2 = 232 col3 = 332 ) ).", rel: Release.v740sp02},
  {abap: "moo = VALUE #(\n" +
    "FOR j = 1 THEN i + 1 WHILE j <= i_cols (\n" +
    "row = i\n" +
    "col = j ) ).", rel: Release.v740sp05},
  {abap: "r_grid->grid_cells = VALUE #(\n" +
    "FOR i = 1 THEN i + 1 WHILE i <= i_rows\n" +
    "FOR j = 1 THEN j + 1 WHILE j <= i_cols (\n" +
    "row = i\n" +
    "col = j ) ).", rel: Release.v740sp05},
  {abap: "r_cells_alive = lines( FILTER #( cells USING KEY key_alive WHERE alive = abap_true ) ).", rel: Release.v740sp08},
  {abap: "foo = CORRESPONDING #( <ls_data> MAPPING country = coun currency = curr ).", rel: Release.v740sp05},
  {abap: "foo = CORRESPONDING #( <ls_data> MAPPING country = coun currency = curr EXCEPT bar ).", rel: Release.v740sp05},
  {abap: "foo = CORRESPONDING #( <ls_data> MAPPING country = coun currency = curr EXCEPT * ).", rel: Release.v740sp05},
  {abap: "ct_usage[ id = c_excel ]-enabled = abap_false.", rel: Release.v740sp02},
  {abap: "ct_usage[ KEY name id = c_excel ]-enabled = abap_false.", rel: Release.v740sp02},
  {abap: "e_object->mt_toolbar[ function = <tb>-function ]-disabled = abap_true.", rel: Release.v740sp02},
  {abap: "ro_elem = VALUE #( char_table[ char = lv_char ]-elem DEFAULT NEW lcl_lisp_char( lv_char ) ).", rel: Release.v740sp08},
  {abap: "color = VALUE #( ( color-col = color_on ) ).", rel: Release.v740sp02},
  {abap: "foo = VALUE #( ( ) ).", rel: Release.v740sp02},
  {abap: "DATA(message) = COND #( LET len = strlen( i_message ) IN WHEN len > 0 THEN 'sdf' ).", rel: Release.v740sp02},
  {abap: "r_list = VALUE #( FOR <attribute> IN sdf->attributes ( <attribute>-name ) ).", rel: Release.v740sp05},
  {abap: "thesum = thesum + me->matrix[ x ] * i_matrix->matrix[ j ].", rel: Release.v740sp02},
  {abap: "thesum = thesum + me->matrix[ x ][ j ] * i_matrix->matrix[ j ][ y ].", rel: Release.v740sp02},
  {abap: "foo = CONV decfloat16( _num_samples ).", rel: Release.v740sp02},
  {abap: "t->matrix[ 1 ][ 4 ] = i_rotate_about_line->x.", rel: Release.v740sp02},
  {abap: "DATA(asdf) = VALUE zfoo(\n" +
    "FOR ls_sdf IN  lt_sdf WHERE ( classtype = '001' AND classnum = 'SOMETHING' )\n" +
    "( ls_sdf ) ).", rel: Release.v740sp05},
  {abap: "DATA(asdf) = VALUE zfoo(\n" +
  ...
  {abap: "DATA(asdf) = VALUE zfoo(\n" +
    ...
    "( ls_sdf ) ).", rel: Release.v740sp05},
  {abap: "et_blah = VALUE #( field = 2 ( id = c_bac ) ( id = c_gen ) ).", rel: Release.v740sp02},
  {abap: "foo = EXACT #( blah ).", rel: Release.v740sp02},
  {abap: "DATA(lv_end_date) = CONV zcreated( ms_periods-end_d ) + 235959.", rel: Release.v740sp02},
  {abap: "target[] = FILTER #( lt_tab[] IN lt_tab2[] WHERE field = var ).", rel: Release.v740sp08},
  {abap: "lt_result = CORRESPONDING table_type( lt_input DISCARDING DUPLICATES ).", rel: Release.v751},
  {abap: "DATA(result) = REDUCE string(\n" +
    "  INIT text = ``\n" +
    "  FOR i = 0 UNTIL i >= xstrlen( source )\n" +
    "  LET char = CONV string( source+i(1) ) IN\n" +
    "  NEXT text = text && |{ char } | ).", rel: Release.v740sp08},

  {abap: "ro_type = SWITCH #( LET rnd = lo_rnd->get( ) IN rnd\n" +
    "  WHEN 3 THEN zcl_log=>go_error\n" +
    "  WHEN 4 THEN zcl_log=>go_debug ).", rel: Release.v740sp02},
  ...
  {abap: "DATA(new_cells) = VALUE tty_cell(\n" +
    ...
    "IN ( col   = cell-col\n" +
    "     row   = cell-row\n" +
    "     alive = rule( ) ) ).", rel: Release.v740sp05},
  ...
  {abap: "ls_line = CORRESPONDING #( is_data\n" +
    ...
    "    field2 = lv_field2\n" +
    "  EXCEPT\n" +
    "    field3 ).", rel: Release.v740sp05},

  {abap: "lt_range = VALUE #( sign = 'I' option = 'EQ' ( low = 'VAL1' )\n" +
    "( low = 'VAL2' )\n" +
    "option = 'NE' ( low = 'VAL3' )\n" +
    "( low = 'VAL4' ) ).", rel: Release.v740sp02},
...
const ok = [
  {abap: `lv_ref_bgqc_name->* = me->ms_item-obj_name.`, rel: Release.v756},
  {abap: `DATA(sdf1) = CAST ty( val )->*.`, rel: Release.v740sp08},
  {abap: `DATA(sdf) = VALUE tystru( sdf = CAST tynumc( foo )->* ).`, rel: Release.v740sp08},
];

statementVersionOk(ok, "MOVE", Statements.Move);

const versionsFail = [
  {abap: "lv_foo *= 2.", rel: Release.v702},
  {abap: `move 2 to lv_foo.`, rel: Release.Newest, langVer: LanguageVersion.Cloud},
];

statementVersionFail(versionsFail, "MOVE");

const v757dyn = [
  {abap: "result = ref->(comp_name).", rel: Release.v757},
  {abap: "result = ref->('comp').", rel: Release.v757},
  {abap: "result = ref->(`comp`).", rel: Release.v757},
  {abap: "result = ref->(comp_name)->*.", rel: Release.v757},
  {abap: "result = struc-(comp_name).", rel: Release.v757},
  {abap: "result = struc-(comp_name)->*.", rel: Release.v757},
  {abap: "result = struc-('comp')->*.", rel: Release.v757},
  {abap: "result = ref->(comp_name->*).", rel: Release.v757},
  {abap: "result = obj->meth( )-(comp_name)->*.", rel: Release.v757},
  {abap: "result = obj->meth( )->(comp_name).", rel: Release.v757},
  {abap: "DATA(result) = ref->(comp_name).", rel: Release.v757},
  {abap: "DATA(result) = struc-(comp_name)->*.", rel: Release.v757},
  {abap: "ref->(comp_name) = 5.", rel: Release.v757},
  {abap: "struc-(comp_name) = 5.", rel: Release.v757},
  {abap: "ref->(comp_name)->* = 5.", rel: Release.v757},
  {abap: "struc-(comp_name)->(attr_name) = 5.", rel: Release.v757},
  {abap: "itab[ 1 ]->(comp_name) = 5.", rel: Release.v757},
  {abap: "itab[ 1 ]->(comp_name)->* = 5.", rel: Release.v757},
  {abap: "result = itab[ 1 ]->(comp_name).", rel: Release.v757},
  {abap: "result = itab[ 1 ]->(comp_name)->*.", rel: Release.v757},
  {abap: "struc-(s_name)-(comp_name) = 5.", rel: Release.v757},
];
...
const v757dynFail = [
  ...
  {abap: "result = struc-( comp).", rel: Release.v757},
  {abap: "result = struc-(comp ).", rel: Release.v757},
  {abap: "result = obj->meth( )-( comp )->*.", rel: Release.v757},
  {abap: "DATA(foo) = bar->( bar ).", rel: Release.v757},
  {abap: "DATA(foo) = bar->( bar).", rel: Release.v757},
  {abap: "DATA(foo) = bar->(bar ).", rel: Release.v757},
  {abap: "DATA(foo) = bar-( bar).", rel: Release.v757},
  {abap: "ref->( comp ) = 5.", rel: Release.v757},
  {abap: "ref->( comp) = 5.", rel: Release.v757},
  {abap: "struc-( comp ) = 5.", rel: Release.v757},
  {abap: "struc-(comp ) = 5.", rel: Release.v757},
];
