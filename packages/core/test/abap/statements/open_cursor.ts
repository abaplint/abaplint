...
const optionsVersions = [
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab1 ORDER BY PRIMARY KEY OPTIONS USING ALL CLIENTS.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS PRIVILEGED ACCESS.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS BYPASSING BUFFER.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS CONNECTION foo.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS USING ALL CLIENTS PRIVILEGED ACCESS.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS USING ALL CLIENTS BYPASSING BUFFER.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS USING ALL CLIENTS CONNECTION foo.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS USING ALL CLIENTS PRIVILEGED ACCESS BYPASSING BUFFER.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS USING ALL CLIENTS PRIVILEGED ACCESS CONNECTION foo.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS USING ALL CLIENTS BYPASSING BUFFER CONNECTION foo.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS USING ALL CLIENTS PRIVILEGED ACCESS BYPASSING BUFFER CONNECTION foo.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS PRIVILEGED ACCESS BYPASSING BUFFER.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS PRIVILEGED ACCESS CONNECTION foo.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS BYPASSING BUFFER CONNECTION foo.`, rel: Release.v816},
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS PRIVILEGED ACCESS BYPASSING BUFFER CONNECTION foo.`, rel: Release.v816},
];

statementVersion(optionsVersions, "OPEN CURSOR OPTIONS clause", Statements.OpenCursor);

const optionsVersionsFail = [
  {abap: `OPEN CURSOR @DATA(cursor) FOR SELECT * FROM ztab OPTIONS USING ALL CLIENTS.`, rel: Release.v758},
];
