...
WRITE / timestamp.`;
  ...
  it.skip("error cannot define length and decimals for date", () => {
    const abap = `
...
foo = |AA|.`;
  ...
  it("curly bracket constant to xstring", () => {
    const abap = `
CLASS lcl DEFINITION.
  PUBLIC SECTION.
    METHODS bar IMPORTING foo TYPE xstring.
ENDCLASS.
...
ENDCLASS.`;
  ...
  it("non hex character constant to xstring", () => {
    const abap = `
CLASS lcl DEFINITION.
  PUBLIC SECTION.
    METHODS bar IMPORTING foo TYPE xstring.
...
ENDCLASS.`;
  ...
  it("incompatible, passing hex to int8", () => {
    const abap = `
CLASS lcl DEFINITION.
  PUBLIC SECTION.
...
ENDCLASS.`;
  ...
  it("ok, passing calculated hex to int8", () => {
    const abap = `
CLASS lcl DEFINITION.
  PUBLIC SECTION.
    METHODS foo IMPORTING bar TYPE int8.
...
CLASS lcl DEFINITION.
  PUBLIC SECTION.
    CLASS-METHODS run IMPORTING input TYPE ty_empty.
ENDCLASS.
...
START-OF-SELECTION.
  DATA def TYPE STANDARD TABLE OF ty WITH DEFAULT KEY WITH HEADER LINE.
  lcl=>run( CONV #( def[] ) ).`;
    const issues = runProgram(abap);
    expect(issues[0]?.getMessage()).to.equal(undefined);
  });
...
SELECT vkorg vtext FROM tvkot
  ...
  WHERE vkorg IN ( SELECT foo FROM voided ).
ENDSELECT.`;
    const issues = runProgram(abap);
    expect(issues[0]?.getMessage()).to.equal(undefined);
  });
...
DATA foo1 TYPE ty_cedi.
DATA foo2 TYPE ty_t001l.
...
CLASS lcl DEFINITION.
  PUBLIC SECTION.
    TYPES: BEGIN OF ty,
             sel TYPE string,
           END OF ty.
    TYPES tttty TYPE STANDARD TABLE OF ty WITH EMPTY KEY.
...
SELECT foo, bar
  ...
  it("error, SELECT IN subselect checks source", () => {
    const abap = `
DATA rt TYPE also_voided.
SELECT foo, bar
  FROM voided1 JOIN t001w ON voided1~locnr = t001w~kunnr
  APPENDING TABLE @rt
  WHERE t001w~werks IN ( SELECT werks FROM voided2 WHERE sdffsd EQ @does_not_exist ).`;
    const issues = runProgram(abap);
    expect(issues[0]?.getMessage()).to.contain(`"does_not_exist" not found`);
  });
...
START-OF-SELECTION.
  ...
  it("CREATE OBJECT, date not compatible with c length 8", () => {
    const abap = `
CLASS lcl DEFINITION.
  PUBLIC SECTION.
    TYPES ty TYPE c LENGTH 8.
    METHODS constructor IMPORTING bar TYPE ty.
ENDCLASS.
