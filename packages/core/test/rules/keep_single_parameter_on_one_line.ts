...
const fixTests = [
  {input: `DATA(unique_list) = remove_duplicates(
    list ).`, output: "DATA(unique_list) = remove_duplicates( list )."},
  {input: `remove_duplicates(
  CHANGING
    list = list ).`, output: "remove_duplicates( CHANGING list = list )."},
];

testRuleFix(fixTests, KeepSingleParameterCallsOnOneLine);