...
import {OpenCursor} from "./statements/open_cursor";
import {FetchNextCursor} from "./statements/fetch_next_cursor";
import {CloseCursor} from "./statements/close_cursor";
...
import {GetPermissions} from "./statements/get_permissions";
...
if (Object.keys(map).length === 0) {
  ...
  addToMap(new GetPermissions());
  addToMap(new ReadDataset());
  addToMap(new ModifyEntities());
  addToMap(new CommitEntities());
  addToMap(new CallKernel());
  addToMap(new OpenCursor());
  addToMap(new FetchNextCursor());
  addToMap(new CloseCursor());
  addToMap(new Search());
