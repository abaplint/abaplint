import {seqs, opts, optPrios, altPrios, alts, Expression} from "../combi";
...
export class FormParamType extends Expression {
  public getRunnable(): IStatementRunnable {
    ...
    const tabseq = seqs(table, optPrios(seqs("OF", TypeName)));

    const ret = seqs(optPrios(alts("REF TO", "LINE OF")),
                     TypeName,
                     opts(def));

    const like = seqs("LIKE", optPrios(alts("REF TO", "LINE OF")),
                      FieldChain);
