import {ver, seq, tok, alt, starPrio, altPrio, Expression, AlsoIn} from "../combi";
...
export class SQLIn extends Expression {
  public getRunnable(): IStatementRunnable {
    ...
    const listOld = seq(tok(WParenLeft), alt(ver(Release.v740sp05, short, {also: AlsoIn.OpenABAP}), val), starPrio(seq(",", val)), altPrio(tok(ParenRight), tok(ParenRightW), tok(WParenRightW)));
    ...
    const listNeww = ver(Release.v740sp02, listNew, {also: AlsoIn.OpenABAP});
