...
export abstract class Structure  {
  ...
  public runFile(file: ABAPFile, statements?: StatementNode[]): {issues: Issue[], node?: StructureNode} {
    ...
    if (result.error) {
      return {issues: [new Issue({file, key: "structure", message: result.errorDescription})], node: undefined};
    }
    if (result.unmatched.length > 0) {
      const statement = result.unmatched[0];
      const descr = "Unexpected " + statement.get().constructor.name.toUpperCase();
      return {issues: [new Issue({file, message: descr, key: "structure", start: statement.getStart()})], node: undefined};
    }
