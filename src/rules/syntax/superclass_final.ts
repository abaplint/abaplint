  ...
  public runParsed(file: ABAPFile, reg: Registry, obj: IObject) {
    ...
    for (const definition of file.getClassDefinitions()) {
      ...
      if (found === undefined) {
        ...
        const issue = new Issue({file, message, key: this.getKey(), start: definition.getPosition()});
        output.push(issue);
        continue;
      }
      if (found.isFinal()) {
        const issue = new Issue({file, message: this.getDescription(), key: this.getKey(), start: definition.getPosition()});
        output.push(issue);
      }
    }
