import {Statement} from "./statement";
import * as Reuse from "./reuse";
import * as Combi from "../combi";

let str = Combi.str;
let seq = Combi.seq;
let alt = Combi.alt;
let opt = Combi.opt;

export class Communication extends Statement {

  public static get_matcher(): Combi.IRunnable {
    let length = seq(str("LENGTH"), new Reuse.Target());

    let init = seq(str("INIT ID"), new Reuse.Source(), str("DESTINATION"), new Reuse.Target());
    let allocate = seq(str("ALLOCATE ID"), new Reuse.Source());
    let send = seq(str("SEND ID"), new Reuse.Source(), str("BUFFER"), new Reuse.Target(), opt(length));
    let deallocate = seq(str("DEALLOCATE ID"), new Reuse.Source());
    let accept = seq(str("ACCEPT ID"), new Reuse.Source());

    let receive = seq(str("RECEIVE ID"),
                      new Reuse.Source(),
                      str("BUFFER"),
                      new Reuse.Source(),
                      str("DATAINFO"),
                      new Reuse.Target(),
                      str("STATUSINFO"),
                      new Reuse.Target(),
                      str("RECEIVED"),
                      new Reuse.Target());

    return seq(str("COMMUNICATION"),
               alt(init, allocate, send, deallocate, receive, accept));
  }

}